...
migrate:
	docker compose exec api python3.10 manage.py migrate

makemigrations:
	docker compose exec api python3.10 manage.py makemigrations

superuser:
	docker compose exec api python3.10 manage.py createsuperuser

collectstatic:
	docker compose exec api python3.10 manage.py collectstatic --no-input --clear
...
estate-db:
	docker compose exec postgres-db psql --username=kene --dbname=estate
